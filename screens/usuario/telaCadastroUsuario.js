...
const TLCadastroUsurio = () => {
  ...
  const handleCadastro = async () => {
    ...
    let { data, error } = await supabase.from('usuariospi').insert([
      {
        id: userId,
        nome: nome,
        email: email,
    ...
    if (error) {
      ...
      navigation.navigate('TLLoginUsuario');
    }
  };

  return (
    <View style={styles.tlCadastroUsurio}>
      <Image
      ...
      {/* Já tenho uma conta */}
      <Pressable onPress={() => navigation.navigate("TLLoginUsuario")}>
...
export default TLCadastroUsurio;
